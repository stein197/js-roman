...
export function parse(num: string): number {
	...
	for (let i = 0, char = num[i], charUppercased = char.toUpperCase(), nextChar = num[i + 1], curCharCount = 1; i < num.length; i++, char = num[i], charUppercased = char?.toUpperCase(), nextChar = num[i + 1], curCharCount = char === num[i - 1] ? curCharCount + 1 : 1) {
		...
		if (!digit)
			throw new SyntaxError(`Cannot parse "${num}" at ${i}: the character "${char}" is not valid roman digit`);
		if (MAX_CHARS < curCharCount)
			throw new SyntaxError(`Cannot parse "${num}" at ${i}: the character "${char}" occurs more than ${MAX_CHARS} times in a row`);
		...
		if ((isLess && curToken) || !isLess) {
			...
			if (token.length === 2) {
				...
				if (isDigitMultipleOfFive(lowChar))
					throw new SyntaxError(`Cannot parse "${num}" at ${i}: lower digit in token "${num.substring(i - 1, i + 1)}" cannot be a multiple of five`);
				const maxHigh = lowValue * 10;
				if (maxHigh < highValue)
					throw new SyntaxError(`Cannot parse "${num}" at ${i}: higher digit in token "${num.substring(i - 1, i + 1)}" is too high`);
			}
			...
			if (lastTokenValue < tokenValue || lastTokenValue <= tokenValue && lastToken.length === 2 || lastToken.length === 2 && lastTokenValue.toString().length === tokenValue.toString().length)
				throw new SyntaxError(`Cannot parse "${num}" at ${i}: token "${num.substring(i - 1, i + 1)}" cannot be greater than the previous one`);
			result += tokenValue;
